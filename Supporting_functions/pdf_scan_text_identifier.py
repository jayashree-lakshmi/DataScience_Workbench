...
# Example: python check_pdf_scan_or_text.py filename.pdf
...
# Local RUn should be false in  Production
#LOCAL_RUN = False
LOCAL_RUN = True
...
def convert_pdf_into_text(pdf_object,
    ...
    :return:
    """
    print("Checking the PDF: `{}` if it's Text based or Scan based PDF".format(local_filename))
    ...
    try:
        if type_file:
            ...
            if pdf_object.isEncrypted:
                pdf_object.decrypt('')
            print("PDF Object is {}".format(pdf_object.documentInfo))
            ...
            if output:
                ...
                print(" `{}` fonts have been detected for the PDF: {}".format(output, local_filename))
                ...
                if output < PDF_FONT:
                    print("The PDF: `{}` detected only `{}` fonts using pdffonts, which classifies into Scan PDF".format(local_filename, output))
            ...
            try:
                ...
                if pdf_object.documentInfo:
                    ...
                    if pdf_creator_type.strip().lower() in ('canon', 'kodak', 'smart touch', 'xerox', 'greypad ccm platform', 'jasperreports (fullstatementrpt)'):
                        print("The PDF: `{}` has been detected as a `Scan based PDF` and will be converted to a text file using OCR technology".format(local_filename))
                        open(output_filename, 'w').close()
		        ...
		        # Run conversion with no lattice param
                    ...
                    # Run conversion with no lattice param
                    print("The PDF: `{}`  will be converted to text using OCR technology".format(local_filename))
                    open(output_filename, 'w').close()
                    #tabula.convert_into(local_filename, output_filename,
                    #                    output_format=output_format,
                    #                    pages=pages,
                ...
                print("The PDF: `{}` has detected some errors: `{}` and will be converted to a text file using OCR technology".format(local_filename, pdferror))
                open(output_filename, 'w').close()
            return pdf_object.documentInfo,output

    except IOError:
        print("The PDF: `{}`  not found, will be converted to text using OCR technology".format(local_filename))
        open(output_filename, 'w').close()
...
def concatenate_files(final_output_filename, file_extension):
    ...
    :return:
    """
    print("Concatenation of files for `{}` ".format(file_extension))
    try:
...
def check_scan_pdf(local_filename, output_filename, type_file):
    ...
    try:
        ...
        try:
            ...
            print("PDF File: {} is encrypted/Password Protected.".format(local_filename))
            open(output_filename, 'w').close()
        ...
        if page_count >= max_page_buffer:
            ...
            print("The PDF `{}` has `{}` pages and has been split into `{}` iterations.".format(local_filename, page_count, no_of_pages_iterate))
            if left_out_pages:
                no_of_pages_iterate += 1
            for page in range(0, no_of_pages_iterate):
                print("PDF Iteration page: `{}`".format(page))
                if page == 0:
                    page_start = 1
        ...
        file_empty_check = check_file_is_empty(output_filename)
        if file_empty_check:
            pdf_type = "Scanned PDF"
            print("The PDF: `{}` has been detected as a `Scan based PDF` and has been converted to a text file using OCR technology".format(local_filename))
            return output_filename, "Scanned_PDF", output, filen
        else:
            pdf_type = "Text based PDF"
            print("The PDF: `{}` has been detected as a normal `Text based PDF` and has been converted to a text file using Tabula".format(local_filename))
            if FILE_ENCRYPTED:
...
def main():
    if LOCAL_RUN:
        ...
        print("The file: {} has been detected as  `{}`".format(output_filename, data_type))
